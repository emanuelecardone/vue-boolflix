<template>
    ...
    <main class="w-100 d-flex justify-content-center align-items-center">
        ...
        <div v-else-if="apiLoaded" class="cards_wrapper w-100 h-100 d-flex flex-wrap">
            <Card v-for="movie,index in userMovies" :key="index" class="w-50 border border-2 border-dark p-2" :title="movie.title" :originalTitle="movie.original_title" :language="movie.original_language" :rate="movie.vote_average" />
        </div>
